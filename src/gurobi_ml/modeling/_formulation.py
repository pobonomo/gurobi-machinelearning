...
class _Formulation:
    ...
    *args.
    ...
    within the model construction method or function of a _SubModel itself.
    ...
    def constrs(self):
        ...
        if self._firstconstr:
            return self.gp_model.getConstrs()[
    ...
    def _close(self, before, name):
        ...
        Record all added modeling objects in _SubModel and prefix their names.
        """
        ...
        self.gp_model._modeling_data.push_name_handler(before[1])

        if self._no_recording:
            return

        # record all newly added modeling objects
        self._record(self.gp_model, before[0])

        name_handler = self.gp_model._modeling_data.name_handler
        if name_handler is None:
            name_handler = NameHandler()
            self.gp_model._modeling_data.push_name_handler(name_handler)

        if name != "":
            # prefix names of newly created modeling objects
            name = name_handler.get_name(self, name)
            self._name = name
            prefix_names(self.gp_model, self.vars, "VarName", name)
            prefix_names(self.gp_model, self.constrs, "ConstrName", name)
            prefix_names(self.gp_model, self.qconstrs, "QCName", name)
            prefix_names(self.gp_model, self.genconstrs, "GenConstrName", name)
    ...
    def remove(self):
        """Remove this from the gurobipy model."""
        if self.gp_model:
            self.gp_model.remove(self.vars)
            self.gp_model.remove(self.constrs)
            self.gp_model.remove(self.qconstrs)
            self.gp_model.remove(self.genconstrs)
            self.gp_model.remove(self.sos)
            if self._first_callback:
                beg = self.gp_model.getCallbackIndex(self._first_callback)
                end = self.gp_model.getCallbackIndex(self._last_callback)
                self.gp_model.removeCallbacks(beg, end)
