...
class TestIssue496(unittest.TestCase):
    ...
    def test_misic_formulation_fix(self):
        """Verify that the Mišić formulation with safety_floor fixes the numerical issue."""
        env = gp.Env()
        m = gp.Model(env=env)
        m.Params.OutputFlag = 0
        x_vars = m.addMVar(shape=5, lb=-10.0, ub=10.0, name='x')
        ...
        # Use misic formulation with safety_floor to avoid the issue
        add_lgbm_booster_constr(m, self.model, x_vars, y_var, formulation="misic", safety_floor=1e-5)
        m.update()
        ...
        for x0 in [0.0, 1e-3, 1.0, 5.0]:
            ...
            self.assertAlmostEqual(lgbm_score, mip_score, places=5, 
